...
class FinanceController < ApplicationController
  ...
  def update_monthly_report
    ...
    @expense_total = FinanceTransaction.expense_total(@start_date,@end_date)
    @income_total = FinanceTransaction.income_total(@start_date,@end_date)
  ...
  def fees_defaulters_details
    @batchs = Batch.active
    @dates = []
    
    @batchs.each do |b|
      @dates = b.fee_collection_dates
    end
  end
  ...
  def fees_defaulters_students_details
    #@batch   = Batch.find(params[:batch_id])
    @date = FinanceFeeCollection.find(params[:date])
    @dates = []
    @students = []
    ...
    @dates = FinanceFeeCollection.all(:conditions => ["name = '#{@date.name}' and is_deleted = 0"])
    
    
    @dates.each do |d|
      #@students << d.students.all
      @students += Student.all(:conditions => ["batch_id = #{d.batch_id}"])
    end
   
    @dates.each do |d|
      @finance_fee = FinanceFee.find(:all, :conditions => ["fee_collection_id = #{d.id}"])
      @finance_fee.each do |f|
        if f.is_paid == false
          @student = Student.find(:all, :conditions => ["id = #{f.student_id}"])
          if !@student.empty?
            @defaulters += @student
            @fee_collection = FinanceFeeCollection.find(d)
            @fee_category = FinanceFeeCategory.find(@fee_collection.fee_category_id,:conditions => ["is_deleted IS NOT NULL"])
            @fee_particulars = @fee_category.fees(@student[0])
            total_fees = 0
            @fee_particulars.each do |p|
              total_fees += p.amount
            end
            @defaulter_fee_due << total_fees
            @total_fee_due += total_fees
          end
        end
      end
    end
    ...
    #@students = @date.students.all
    #@defaulters = @students.reject{|s| s.check_fee_pay(@date)}
    render :update do |page|
      page.replace_html "student", :partial => "student_defaulters_details"
    end
  end
  ...
  def fee_defaulters_details_pdf
    #@batch   = Batch.find(params[:batch_id])
   #@date = FinanceFeeCollection.find(params[:date])
    #@students = @date.students.all
    #@students = @students.reject{|s| s.check_fee_pay(@date)}
    #@currency_type = Configuration.find_by_config_key("CurrencyType").config_value
    ...
    render :pdf => 'fee_defaulters_details_pdf'
  ...
  def report_compare
    ...
    @income_total = FinanceTransaction.income_total(@start_date,@end_date)
    @grand_total = FinanceTransaction.grand_total(@start_date,@end_date)
    @expense_total2 = FinanceTransaction.expense_total(@start_date2,@end_date2)
    @income_total2 = FinanceTransaction.income_total(@start_date2,@end_date2)
